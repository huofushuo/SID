...
class GenerationMixin:
    ...
    def _update_model_kwargs_for_generation(
        ...
        if not is_encoder_decoder:
            ...
            if "attention_mask_icd" in model_kwargs:
                attention_mask_icd = model_kwargs["attention_mask_icd"]
                model_kwargs["attention_mask_icd"] = torch.cat(
                    [attention_mask_icd, attention_mask_icd.new_ones((attention_mask_icd.shape[0], 1))], dim=-1)
        else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if model_kwargs.get("input_ids_cd", None) is not None:
            if model_kwargs.get("attention_mask_icd", None) is None and requires_attention_mask and accepts_attention_mask:
                model_kwargs["attention_mask_icd"] = self._prepare_attention_mask_for_generation(
                    model_kwargs["input_ids_cd"], generation_config.pad_token_id, generation_config.eos_token_id
                )
        
        if model_kwargs.get("inputs_embeds_cd", None) is not None:
            if model_kwargs.get("attention_mask_icd", None) is None and requires_attention_mask and accepts_attention_mask:
                model_kwargs["attention_mask_icd"] = self._prepare_attention_mask_for_generation(
                    model_kwargs["inputs_embeds_cd"], generation_config.pad_token_id, generation_config.eos_token_id
                )
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # statelessmodel_kwargs["past_key_values"]
        ...
        **model_kwargs,
        ...
        while True:
            ...
            for candidate_id in range(num_attn_candidates):
                ...
                # attn_square = torch.cat([attn_square, outputs_tmp.attentions[-1].clone()], -2) # [batch_size * num_beams, num_head, q+1, kv+1]
                # attn_last.append(attn_square.max(1, keepdim=True).values.data) # [batch_size * num_beams, 1, q+1, kv+1]
